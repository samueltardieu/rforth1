...
\ send a can message of payload 4
: emit5 ( il ih d1 d2 d3 d4 d5 -- )
  5 set-payload
  begin TXB0TXREQ bit-clr? until
...
local_addr forth_mask or 8 read16 dup . 50 8 emit5 
; inline

: send-back
." Sent back" cr
\ stdid acc vmax tvmax dec payload
local_addr back_mask or  8 read16 dup . 50 8 emit5
; inline
